...
public class TrignometricdiagramEvaluator extends Evaluator {
    ...
    public MarkSheet evaluate(AbstractDiagramStructure studentStructure,
        ...
        for (SubQuestion subQuestion : rubricRules.getSubQuestions()){
            ...
            subQmarkSheet = new SubMarkSheet();
            subQmarkSheet.setTotalMark(totalSubQ);
            subQmarkSheet.setFeedBack(subQfeedback);
            subQmarkSheet.setPartitialMark(marks);
            subQmarkSheet.setGainedMark(totalMarks);
    ...
    private Mark checkMarkedLength(Condition condition){
        ...
        if((teacherLengthCount == lengthCount)&& (lengthCount!= 0 )){
            ...
            subQfeedback = subQfeedback + "Length labels are marked correctly in the student's answer \n";
            mark.setFeedBack("Length labels are marked correctly in the student's answer");

            return  mark;
        }
        else{
            subQfeedback = subQfeedback  + "Length labels are not marked in the Diagram correctly \n";
